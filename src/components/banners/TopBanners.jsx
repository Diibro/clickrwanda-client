          ...
          <div className="full flex items-center justify-center md:grid grid-cols-2 overflow-hidden gap-[10px]">
               {
                    ...
                    activeBanners.map((banner, index) => <a key={`top-banner-${index}`} href={banner.destLink} target="_blank" rel="noreferrer" className="w-full rounded-[8px]"  ><img src={banner.srcLink}  width={800} height={100} className="w-full aspect-[800/100] rounded-[8px] object-contain " /></a>)
                    :null
               }
          </div>
     )
